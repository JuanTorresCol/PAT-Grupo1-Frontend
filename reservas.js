...
async function cargarReservas(token) {
    try {
        ...
        mostrarFilaMensaje("No se han podido cargar tus reservas.");
    }
}
...
function aplicarBusqueda() {
    ...
    estadoGlobal.reservasFiltradas = estadoGlobal.reservasOriginales.filter((reserva) => {
        return String(reserva.id).toLowerCase().includes(valor);
    });
...
function renderizarTabla() {
    ...
    reservasPagina.forEach((reserva) => {
        ...
        fila.addEventListener("click", () => {
            ...
            window.location.href = `viewReserva.html?id=${encodeURIComponent(reserva.id)}`;
        });

        fila.innerHTML = `
            <td>
                <strong>#${escapeHTML(reserva.id)}</strong>
            </td>
            <td>
                ${escapeHTML(reserva.nombrePista)}
            </td>
            <td>
